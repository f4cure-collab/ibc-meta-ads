...
def api_campaigns():
    ...
    try:
        ...
        # v10: agrupamento de eventos passou a separar por 14 dias sem veiculacao
        # (antes: distancia entre datas de criacao). O agrupamento e derivado do
        # cache RAW, entao o bump so re-deriva — nao gera chamada Meta.
        cache_key = f"campaigns_v10_{camp_type}_{camp_status}_{date_from}_{date_to}"
        if not force:
            cached = get_cached(cache_key)
            if cached:
...
def api_resumo():
    ...
    try:
        ...
        def _aggregate_type(ct, d_from, d_to, want_detail=True):
            ...
            key_camp = f"campaigns_v10_{ct}_all_{d_from}_{d_to}"
                ...
                try:
                    with app.test_client() as client:
                        ...
                        for seg_from, seg_to in segments:
                            sk_camp = f"campaigns_v10_{ct}_all_{seg_from}_{seg_to}"
...
def _monthly_cache_keys(dt_from, dt_to):
    ...
    for ct in VALID_CAMP_TYPES:
        keys.append(f"campaigns_v10_{ct}_all_{dt_from}_{dt_to}")
...
def _warmup_camp_type(ct, days_list, dt_to):
    ...
    with app.test_client() as client:
        ...
        for days in days_list:
            ...
            try:
                k_camp = f"campaigns_v10_{ct}_all_{dt_from}_{dt_to}"
